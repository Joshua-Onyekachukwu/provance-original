...
/**
 * The canonical full migration manifest — every file in supabase/migrations/
 * in dependency (numeric) order, marker-delimited so check-migration-
 * convergence.mjs (CI) can parse it and so a regenerate is a clean replace.
 *
 * Deliberately independent of the live probe state: the founder checklist
 * above shrinks as migrations land, but the manifest is the stable "what
 * exists in the repo" contract the CI gate compares the real migration dir
 * against. Keep the format in sync with check-migration-convergence.mjs.
 */
function buildManifestSection() {
  const files = readdirSync(new URL('../../supabase/migrations', import.meta.url))
    ...
    .sort();
  const lines = [];
  lines.push('<!-- BEGIN MIGRATION MANIFEST — canonical file set + order; regenerated by `node scripts/validate-migrations.mjs --runbook` or `node scripts/check-migration-convergence.mjs --fix`; do not hand-edit between markers -->');
  lines.push('| # | Migration file |');
  lines.push('| - | -------------- |');
  files.forEach((f, i) => lines.push(`| ${i + 1} | \`${f}\` |`));
  lines.push('<!-- END MIGRATION MANIFEST -->');
  return lines.join('\n');
}
...
if (args.includes('--runbook')) {
  ...
  // (see buildRunbookSection) plus the canonical migration manifest. Always
  // exits 0 — it is a doc generator, not a gate.
  console.log(buildRunbookSection(rows));
  console.log('');
  console.log(buildManifestSection());
  process.exit(0);
}
