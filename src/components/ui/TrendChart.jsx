...
import { CHART_H, CHART_W, PAD, buildChartGeometry, buildHitAreaCells } from './chartGeometry.js'
...
//   emptyDescription
//   labels          optional { scans, completed, failed } to relabel the
//                   legend + hover readout for non-scan series (e.g. a queue
//                   throughput trend: Processed / Completed / Failed)
...
export default function TrendChart({
  ...
  const labelScans = labels.scans || 'Scans'
  const labelCompleted = labels.completed || 'Completed'
  const labelFailed = labels.failed || 'Failed'
  ...
  return (
    <div className={`rounded-3xl border border-stone-light bg-white-warm p-6 shadow-sm ${className}`}>
      ...
      <ChartHoverReadout
        ...
        items={hovered ? [
          { key: 'scans', text: `${hovered.scans} ${labelScans.toLowerCase()}` },
          { key: 'completed', text: `${hovered.completed} ${labelCompleted.toLowerCase()}`, className: 'text-emerald-600' },
          { key: 'failed', text: `${hovered.failed} ${labelFailed.toLowerCase()}`, className: 'text-rose-500' },
        ] : []}
      />
      ...
      <div className="relative">
        ...
        {/* HTML axis labels — shared overlay, crisp at any container width */}
        <ChartAxisLabels
          geometry={geometry}
          points={points}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2">
        <span className="inline-flex items-center gap-2 text-xs text-charcoal-mid">
          ...
          {labelScans} ({totalScans.toLocaleString()})
        </span>
        <span className="inline-flex items-center gap-2 text-xs text-charcoal-mid">
          <span className="h-0.5 w-5 rounded-full border-t-2 border-dashed border-emerald-500" />
          {labelCompleted} ({totalCompleted.toLocaleString()})
        </span>
        <span className="inline-flex items-center gap-2 text-xs text-charcoal-mid">
          <span className="h-2 w-2 rounded-full bg-rose-500" />
          {labelFailed} ({totalFailed})
        </span>
        {last ? (
