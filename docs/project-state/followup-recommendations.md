...
| 2026-08-10 | Live scan-flow walk (migration probe) | Apply the whole pending migration set (0005, 0007, 0008, 0009 + 0010, 0011, 0014, 0016, 0020 — all `PGRST205`/`42703` in live non-head probes; only 0001/0002/0004 landed) via `DATABASE_URL` in `backend/.env.local` or dashboard SQL-editor pastes, then re-run the live scan round-trip; `POST /v1/scans` currently 503s and `invite-accept.e2e-spec.ts` fails on the missing org tables | Open |
