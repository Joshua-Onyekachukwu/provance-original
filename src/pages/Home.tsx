    ...
    <main>
      <section className="noise-overlay px-6 pb-16 pt-14 lg:px-10 lg:pb-24 lg:pt-20">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          ...
          <div className="section-fade section-fade-delay-1 float-panel space-y-4">
            <div className="surface-panel rounded-[2rem] p-6">
              <div className="flex items-start justify-between gap-4 border-b border-white/8 pb-5">
                <div>
                  <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-lime-100/70">Live Result Surface</p>
                  <h2 className="mt-4 font-display text-4xl leading-none text-sand">Likely synthetic</h2>
                  <p className="mt-3 max-w-lg text-sm leading-7 text-zinc-400">
                    A mature verification system does not just produce a score. It produces a judgment, its evidence, its limitations, and the artifact a team can take forward.
                  </p>
                </div>
                <div className="shimmer-line rounded-full border border-lime-300/15 bg-lime-300/[0.07] px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-lime-100">
                  92% confidence
                </div>
              </div>

              <div className="mt-4 flex items-center gap-3 rounded-[1.25rem] border border-orange-300/10 bg-orange-300/[0.08] px-4 py-3">
                <span className="h-2.5 w-2.5 rounded-full bg-orange-300" />
                <p className="text-sm text-orange-50/90">
                  Report delivery ready. Evidence appendix, audit metadata, and download actions compiled.
                </p>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={label} className="surface-card rounded-[1.35rem] p-4">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-[1.05fr_0.95fr]">
                <div className="surface-card rounded-[1.5rem] p-5">
                  <p className="font-mono text-[11px] uppercase tracking-[0.26em] text-zinc-500">Signal profile</p>
                  <div className="mt-4 space-y-3">
                    {[
                      ['Temporal consistency', 'High'],
                      ['Texture artifact mismatch', 'Supported'],
                      ['Metadata confidence', 'Moderate'],
                    ].map(([label, value]) => (
                      <div key={label} className="flex items-center justify-between rounded-2xl bg-black/18 px-4 py-3">
                        <span className="text-sm text-zinc-300">{label}</span>
                        <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-lime-100">{value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid gap-3">
                  {[
                    ...
                    { icon: Cpu, label: 'Fast processing' },
                    { icon: DatabaseZap, label: 'Team-ready workflow' },
                  ].map((item) => (
                    <div key={item.label} className="surface-card rounded-[1.35rem] p-5">
                      <item.icon className="h-5 w-5 text-lime-100" />
                      <p className="mt-10 text-sm font-semibold tracking-[0.12em] text-white">{item.label}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="px-6 py-28 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <SectionHeading
            ...
            title="The report should feel like the proof, not a screenshot of the proof."
            description="The public site needs a stronger artifact moment. This section anchors the product story in something concrete, premium, and workflow-ready."
          />
          <div className="mt-12">
            <ReportPreview />
          </div>
      ...
      <section className="bg-[rgba(255,248,241,0.72)] px-6 py-28 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <SectionHeading
            ...
            description="This section should tell a buyer exactly how Provance expands from first evaluation to operational adoption, with clear value at every level."
          />
                ...
                >
                  ...
                  <div className="mt-5 space-y-2">
                    {tier.features.map((feature) => (
                      <div
                        key={feature}
                        className={`rounded-2xl px-4 py-3 text-sm ${tier.name === 'Enterprise' ? 'border border-white/8 bg-white/[0.04] text-zinc-200' : 'border border-black/8 bg-white/55 text-[#2d3338]'}`}
                      >
                        {feature}
                      </div>
                    ))}
        ...
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-black/8 bg-[linear-gradient(135deg,rgba(155,180,92,0.18),rgba(255,255,255,0.54),rgba(255,248,240,0.7))] p-8 shadow-[0_24px_70px_rgba(31,26,18,0.08)] lg:p-12">
          ...
          <div className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                to="/docs"
                className="soft-button inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm text-[#17181a]"
              >
                Explore API Path
                <ChevronRight className="h-4 w-4" />
              </Link>
